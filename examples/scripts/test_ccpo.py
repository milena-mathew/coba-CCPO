...
def feedback(index: int, context: Context, action: Action) -> float:
    return tuple((sum(c*a for c,a in zip(context,action)), sum(c-a for c,a in zip(context,action)), sum(c+a for c,a in zip(context,action))))

sim = [ConstrainedSimulation(n_interactions, context, actions, feedback)]

Benchmark(sim).evaluate([ChanceConstrainedOptimizer(constraint=0.1, learning_rate=0.3, learner=VowpalLearner, vw_kwargs={"bag":5, "seed":10}), ChanceConstrainedOptimizer(constraint=0.1, learning_rate=0.3, learner=RandomLearner)]).plot_learners()
